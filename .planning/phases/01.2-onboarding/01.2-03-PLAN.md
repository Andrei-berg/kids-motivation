...
must_haves:
  truths:
    ...
    - "Step 4 (Add Child): parent enters child name + birth year, picks emoji avatar OR uploads a photo, child added to family_members with avatarUrl"
  ...
  artifacts:
    - path: "app/onboarding/page.tsx"
      provides: "7-step wizard component with React state, step persistence, slide transitions, emoji picker + photo upload on step 3"
      min_lines: 250
  ...
  key_links:
    ...
    - from: "app/onboarding/page.tsx (Step 3 photo upload)"
      to: "Supabase Storage avatars bucket"
      via: "supabase.storage.from('avatars').upload() then getPublicUrl()"
      pattern: "storage.*avatars|from\\('avatars'\\)"
    - from: "app/onboarding/page.tsx"
...
<task type="auto">
  <name>Task 1: Update app/onboarding/welcome/page.tsx — redirect to /onboarding</name>
...
This keeps the middleware target valid (/onboarding/welcome) while sending users to the actual wizard page (/onboarding). No loading state needed — redirect is instant.
  </action>
  <verify>
...
<task type="auto">
  <name>Task 2: Create app/onboarding/page.tsx — 7-step wizard (steps 1-5 implemented, steps 6-7 placeholder)</name>
...
- Progress bar at top: small bar (h-2, gray-200 track, emerald-500 fill), with text "Step {current+1} of 7" (text-xs, gray-500, mb-2)
  - Width: `${((currentStep + 1) / 7) * 100}%` — animate with transition-all duration-300
...
**Slide transition:**
- Use CSS classes. When `direction === 'forward'`: slide incoming step from right (translateX(100%) → translateX(0)). When `direction === 'backward'`: from left (translateX(-100%) → translateX(0)).
...
**Step 0 — Welcome:**
- Large emoji: 🌟 (text-6xl, text-center)
- Heading: "Welcome to FamilyCoins" (text-2xl font-bold gray-900)
- Subheading: "Motivate your children with a real reward system. Grades, chores, sport — all tracked and rewarded." (gray-500, text-sm, text-center, max-w-xs mx-auto)
- 3 short bullet points with emoji icons (Claude's discretion on exact copy, keep in Russian or English — use Russian since app is in Russian):
...
- Primary button: "Начать" (emerald-500 bg, white text, rounded-lg, py-3, full-width)
- On click: setCurrentStep(1), setDirection('forward'). NO DB call on step 0.

**Step 1 — Profile:**
- Heading: "Расскажите о себе"
- Subheading: "Как вас зовут?"
- Single text input: "Ваше имя" (placeholder), value = wizardData.parentName
- Primary button: "Далее"
- On click: validate name not empty, call `saveParentProfile(userId, { displayName: name })`, advance step

**Step 2 — Family:**
- Heading: "Создайте вашу семью"
- Subheading: "Введите название вашей семьи"
- Text input: "Например: Семья Ивановых" (placeholder), value = wizardData.familyName
...
**Step 3 — Add Child:**
- Heading: "Добавьте ребёнка"
- Subheading: "Введите данные вашего первого ребёнка"
- Text input: "Имя ребёнка" (value = childName state)
- Number input or Select: "Год рождения" (value = birthYear, current year ± 18 range)
- **Avatar selection (locked decision: emoji grid + optional photo upload):**
  - Section label: "Фото профиля" (text-sm font-medium gray-700, mb-2)
  - Emoji picker grid (8 options in a 4×2 grid, each a button): 👦 👧 🧒 👶 🦸 🧠 ⭐ 🏆. Selected emoji gets emerald ring (ring-2 ring-emerald-500). Default: 👦.
  - Divider below grid: "или" (gray-400, text-xs, text-center, my-2)
  - Photo upload section:
    - Hidden `<input type="file" accept="image/jpeg,image/png,image/webp" />` (ref via useRef)
    - Visible button: "Загрузить фото" (text-sm, border border-gray-200, rounded-lg, px-3 py-2, hover:border-emerald-500) — onClick triggers the hidden file input click
    - When a file is selected (onChange on the hidden input):
      1. Show a thumbnail preview: `URL.createObjectURL(file)` displayed as a small rounded circle (w-12 h-12, object-cover, rounded-full, border-2 border-emerald-500) replacing the emoji grid selection highlight
      2. Store the File in `photoFile` state
      3. Clear the emoji selection (`setSelectedEmoji(null)`)
    - State shape addition: `photoFile: File | null`, `photoPreviewUrl: string | null`
  - Upload logic runs at step submission (not on file select — avoids orphaned uploads):
    - If `photoFile` is set: upload to Supabase Storage before calling `addChildToFamily`
      ```typescript
      ...
      ```
    - If `photoFile` is null and emoji is selected: `avatarUrl = selectedEmoji` (store emoji string as avatarUrl — the family_members.avatar_url column accepts any string)
- Primary button: "Добавить ребёнка"
- On click: validate childName not empty, run upload if photoFile set (see above), call `addChildToFamily(wizardData.familyId, { displayName: childName, birthYear, avatarUrl }, userId)`, store inviteCode if not already stored, advance step

**Step 4 — Invite Parent (or show invite code):**
- Heading: "Пригласите второго родителя"
- Subheading: "Поделитесь этим кодом с супругом(ой)"
- Display family invite code in a styled box: large text, monospace font, letter-spacing wide, emerald border, copy button (📋 icon, on click: navigator.clipboard.writeText(inviteCode), show "Скопировано!" for 2s)
- Instructions: "Ребёнок тоже использует этот код чтобы присоединиться"
- Two buttons: "Пропустить" (gray, text-only) and "Далее" (emerald, primary). Both advance to step 5.
- On "Пропустить": call `updateOnboardingStep(userId, 5)` and advance.
- On "Далее": same.

**Steps 5-6 placeholder** (implemented in Plan 04 — 01.2-04):
- If currentStep >= 5, show a simple loading placeholder: "Загружаем следующий шаг..." — this will be replaced in Plan 04.
...
**Back button behavior:** setDirection('backward'), setCurrentStep(currentStep - 1). Do NOT undo DB changes — just navigate back in the UI.
...
**Loading state:** while submitting, disable the primary button and show a spinner (simple: text changes to "..." or use a CSS border-top spinner, Claude's discretion on implementation).
  ...
  <done>app/onboarding/page.tsx: wizard with progress bar, steps 0-4 fully implemented (Step 3 includes emoji grid + optional photo upload to Supabase Storage), steps 5-6 placeholder. DB calls use lib/onboarding-api.ts. TypeScript clean. Slide direction state tracked.</done>
</task>
...
- Step 0 (Welcome): shows intro content, Начать button advances
- Step 1 (Profile): name input, saves to DB on Next
- Step 2 (Family): family name input, creates family in DB, stores inviteCode
- Step 3 (Add Child): emoji picker grid shown first; "или" divider; "Загрузить фото" button triggers file input; uploading a photo stores it to Supabase Storage avatars bucket and uses public URL as avatarUrl; emoji and photo are mutually exclusive selections
...
3. Step 3: emoji grid selectable with emerald ring highlight; "Загрузить фото" opens file picker; selected photo shows thumbnail preview; on submit, photo uploaded to avatars bucket, public URL stored in family_members.avatar_url
4. Family invite code displayed on step 4 with working copy button
5. Refreshing /onboarding resumes at last completed step
6. Slide transitions work: forward slides in from right, back from left
