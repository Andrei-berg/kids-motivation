...
must_haves:
  truths:
    - "Step 6 (Categories): parent selects from 4 preset category toggle cards; selections are not persisted in Phase 1.2 — Phase 1.3 handles category seeding"
    - "Step 7 (Done): confetti burst fires, success message shows, app navigates to /dashboard after button click"
...
<objective>
Complete the onboarding wizard with steps 6-7: category selection and the Done/confetti screen. Replace the placeholder added in Plan 03. After Done, parent lands on /dashboard — fully onboarded.
...
<!-- Category persistence decision: -->
// The families table in schema-v3.sql does NOT have a categories column.
// DECISION (Option B, per Claude's Discretion in CONTEXT.md): Skip persisting categories in Phase 1.2.
// Show the category toggle UI (REQ-ONB-007), but store nothing to DB in Phase 1.2.
// Phase 1.3 (categories-schedule) will create the real category tables and seed them.
// The only DB call on step 5 completion: updateOnboardingStep(userId, 6).
// IMPORTANT: Document this deferral in SUMMARY.md so the Phase 1.3 executor knows to handle
// category seeding from wizard selections (or simply seed all 4 default categories unconditionally).
...
**Step 5 — Categories:**
- Heading: "Выберите категории активностей"
- Subheading: "Вы сможете добавить больше в настройках"
- 4 toggle cards (2×2 grid on mobile), each with emoji + name + description:
  - 📚 "Учёба" — "Оценки, домашние задания"
  - 🏠 "Дом" — "Уборка комнаты, помощь по дому"
  - ⚽ "Спорт" — "Тренировки, секции, упражнения"
  - ⏰ "Распорядок" — "Режим дня, личная гигиена"
...
- Categories are NOT saved to DB in Phase 1.2 — only DB call is `updateOnboardingStep(userId, 5)` on step advancement. Phase 1.3 will handle category seeding.
- "Продолжить" button (always enabled — can proceed even with all deselected)
...
- On entering this step: fire confetti immediately
  ...
  ```
- Large success emoji: 🎉 (text-6xl, text-center)
- Heading: "Всё готово!" (text-2xl font-bold, text-center)
- Subheading: "Ваша семья создана. Пора начинать!" (gray-500, text-center)
- Summary card (rounded-xl, bg-gray-50, p-4, mt-4): shows what was created:
  - "👤 Родитель: {parentName}"
  - "🏠 Семья: {familyName}"
  - "🔑 Код приглашения: {inviteCode}" (with copy button)
- Primary button: "Перейти в приложение" (emerald-500)
- On click (or auto after 5s — Claude's discretion, but button click is primary): `updateOnboardingStep(userId, 6)` then `router.push('/dashboard')`
...
**Progress bar on step 6:** show "Step 7 of 7" and 100% fill.
  ...
  <done>canvas-confetti installed. Steps 5 (category toggles — UI only, no DB write) and 6 (confetti + done screen) implemented. Wizard reaches /dashboard on completion. TypeScript clean. SUMMARY.md documents category deferral to Phase 1.3.</done>
</task>
...
3. You should be redirected to /onboarding/welcome → then to /onboarding automatically
4. Walk through all 7 steps:
   - Step 1 (Welcome): verify emoji, description text, "Начать" button
   - Step 2 (Profile): enter name, verify it saves (progress bar advances to 2/7)
   - Step 3 (Family): enter family name, verify family created in Supabase Dashboard → Table Editor → families
   - Step 4 (Add Child): enter child name, pick emoji OR click "Загрузить фото" to upload an image; verify family_members row created with role='child' and avatar_url set
   - Step 5 (Invite): verify invite code displayed, copy button works (paste in text editor to confirm)
   - Step 6 (Categories): verify 4 category cards toggle on/off (no DB write expected — this is UI only)
   - Step 7 (Done): verify confetti fires, summary card shows parent name + family name + invite code
5. Click "Перейти в приложение" → verify redirect to /dashboard
6. Reload /onboarding → verify you're sent to /dashboard (onboarding_step = 6, wizard should redirect completed users)
...
<verification>
- Step 5: 4 category toggles, all ON by default, "Продолжить" advances; no DB write for category selections (only updateOnboardingStep)
- Step 6: confetti fires on enter, summary card shows name/family/code, "Перейти в приложение" → /dashboard
- onboarding_step = 6 in user_profiles after completion (verify in Supabase Table Editor)
- Completed users who revisit /onboarding: add a check in useEffect — if onboarding_step >= 6 and user has family_members row, redirect to /dashboard immediately
- Progress bar shows 100% on step 7
- TypeScript: `npx tsc --noEmit` passes
</verification>

<success_criteria>
1. Step 5 shows 4 category toggles with correct emoji and copy (UI only — no DB persistence in Phase 1.2)
2. Step 6 triggers confetti animation
3. Done screen shows family summary with invite code
4. "Перейти в приложение" navigates to /dashboard
