...
export default function LeaderboardPage() {
  ...
  if (!me) return (
    <div style={{ padding: 32, textAlign: 'center', color: T.ink3, fontFamily: T.fBody }}>{t('kidLeaderboardPage.noData')}</div>
  )

  const stats = [
    { k: t('kidLeaderboardPage.coinsWeek'),    me: me.weekScore,      them: sibling?.weekScore ?? 0,      icon: '💰', col: T.sunDeep },
    { k: t('kidLeaderboardPage.xpTotal'),      me: me.xp,             them: sibling?.xp ?? 0,             icon: '⚡', col: T.plum   },
    { k: t('kidLeaderboardPage.coinsTotal'),   me: me.coins,          them: sibling?.coins ?? 0,          icon: '🏦', col: T.teal   },
    { k: t('kidLeaderboardPage.badges'),       me: me.badgeCount,     them: sibling?.badgeCount ?? 0,     icon: '🏆', col: T.coral  },
    { k: t('kidLeaderboardPage.streakRecord'), me: me.longestStreak,  them: sibling?.longestStreak ?? 0,  icon: '🔥', col: T.pink   },
  ]
    ...
    <div style={{ paddingBottom: 110, maxWidth: 500, margin: '0 auto' }}>
      ...
      <div style={{ padding: '12px 16px 0' }}>
        <div style={{ borderRadius: 28, padding: '20px 16px 16px', position: 'relative', overflow: 'hidden', background: `linear-gradient(135deg, #1A1423 0%, #3D2B5C 100%)`, boxShadow: '0 10px 30px rgba(0,0,0,0.25)' }}>
          ...
          <div style={{ textAlign: 'center', fontFamily: T.fBody, fontSize: 11, color: 'rgba(255,255,255,0.7)', fontWeight: 700, letterSpacing: 1.5, marginBottom: 14, position: 'relative' }}>{t('kidLeaderboardPage.familyRating')}</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, position: 'relative' }}>
            <div style={{ flex: 1, textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: T.fBody, fontSize: 10, color: T.sun, fontWeight: 800, letterSpacing: 0.5 }}>
                {sibling ? t('kidLeaderboardPage.leaderLabel', { wins, total: stats.length }) : t('kidLeaderboardPage.youLabel')}
              </div>
            </div>
            {sibling && (
              <>
                ...
                <div style={{ flex: 1, textAlign: 'center' }}>
                  ...
                  <div style={{ fontFamily: T.fBody, fontSize: 10, color: 'rgba(255,255,255,0.6)', fontWeight: 700, letterSpacing: 0.5 }}>{t('kidLeaderboardPage.levelLabel', { level: sibling.level })}</div>
                </div>
              </>
            )}
          </div>
          <div style={{ marginTop: 16, display: 'flex', gap: 3, padding: 3, background: 'rgba(255,255,255,0.1)', borderRadius: 999, position: 'relative' }}>
            {(['week', 'month', 'all-time'] as const).map(r => (
              <button key={r} onClick={() => setRange(r)} style={{ flex: 1, height: 30, borderRadius: 999, border: 'none', cursor: 'pointer', background: range === r ? '#fff' : 'transparent', color: range === r ? T.ink : 'rgba(255,255,255,0.7)', fontFamily: T.fDisp, fontSize: 11, fontWeight: 800 }}>
                {r === 'week' ? t('kidLeaderboardPage.weekTab') : r === 'month' ? t('kidLeaderboardPage.monthTab') : t('kidLeaderboardPage.allTimeTab')}
              </button>
            ))}
          </div>
        </div>
      ...
      {sibling && (
        <div style={{ padding: '18px 16px 0' }}>
          <h3 style={{ margin: '0 0 12px', fontFamily: T.fDisp, fontSize: 20, fontWeight: 900, color: T.ink, letterSpacing: -0.3 }}>{t('kidLeaderboardPage.comparison')}</h3>
              ...
              return (
                <div key={s.k} style={{ background: '#fff', borderRadius: 18, padding: '12px 14px', border: `1.5px solid ${T.line}`, boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    ...
                    {meWins && <span style={{ padding: '2px 8px', borderRadius: 999, background: T.coralSoft, color: T.coralDeep, fontFamily: T.fDisp, fontSize: 10, fontWeight: 900 }}>{t('kidLeaderboardPage.youWin')}</span>}
                  </div>
      ...
      {sibling ? (
        <div style={{ padding: '20px 16px 0' }}>
          <div style={{ background: `linear-gradient(135deg, ${T.tealSoft}, #C6F0EB)`, borderRadius: 20, padding: 16, display: 'flex', gap: 12, alignItems: 'center', border: `1.5px solid ${T.teal}30` }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: T.fDisp, fontSize: 14, fontWeight: 900, color: T.ink }}>{t('kidLeaderboardPage.cheerTitle', { name: sibling.name })}</div>
              <div style={{ fontFamily: T.fBody, fontSize: 12, color: T.ink2, marginTop: 2 }}>{t('kidLeaderboardPage.cheerNote')}</div>
            </div>
            <KMButton tone="teal" size="sm">{t('kidLeaderboardPage.sendStar')}</KMButton>
          </div>
        </div>
      ) : (
        <div style={{ padding: '24px 16px 0', textAlign: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 22, padding: 32, border: `1.5px solid ${T.line}` }}>
            ...
            <div style={{ fontFamily: T.fDisp, fontSize: 16, fontWeight: 800, color: T.ink3, marginTop: 12 }}>{t('kidLeaderboardPage.noMembers')}</div>
          </div>
        </div>
      )}
    </div>
